#ifndef MNV_MULatErrorBand2D_H
#define MNV_MULatErrorBand2D_H 1
...
namespace PlotUtils
{
	...
	class MULatErrorBand2D : public TH2D
	{
		public:
			//! Default constructor 
			MULatErrorBand2D( ) : TH2D() {};

			MULatErrorBand2D( const std::string& name, const TH2D* base, const unsigned int nHists = 1000 );

			//! Add a new Constructor for already Filled vector of Histogram Error Bands
			MULatErrorBand2D( const std::string& name, const TH2D* base, const std::vector<TH2D*>& hists );

			//! Deep copy constructor
			MULatErrorBand2D( const MULatErrorBand2D& h );

			//! Deep assignment operator
			MULatErrorBand2D& operator=( const MULatErrorBand2D& h );

		private:
			//! A helper function which sets variables for the deep copy and assignment
			void DeepCopy( const MULatErrorBand2D& h );
		...
		public:
			...
			Bool_t Add( const MULatErrorBand2D* h1, const Double_t c1 = 1. );

			//! Replace this MULatErrorBand2D's contents with the result of a multiplication of two other MULatErrorBands2D
			Bool_t Multiply( const MULatErrorBand2D* h1, const MULatErrorBand2D* h2, Double_t c1 = 1, Double_t c2 = 1 );

			/*! Replace this MULatErrorBand2D's contents with the result of a division of one MULatErrorBand2D by a TH2
			*/
			Bool_t DivideSingle( const MULatErrorBand2D* h1, const TH2* h2, Double_t c1 = 1, Double_t c2 = 1, Option_t* option="" );

			/*! Replace this MULatErrorBand2D's contents with the result of a division of two other MULatErrorBand2Ds
			*/
			Bool_t Divide( const MULatErrorBand2D* h1, const MULatErrorBand2D* h2, Double_t c1 = 1, Double_t c2 = 1, Option_t* option="" );
		...
		private:
			//!define a class named MULatErrorBand2D, at version 1 
			ClassDef( MULatErrorBand2D, 1 );
	}; //end of MULatErrorBand2D
